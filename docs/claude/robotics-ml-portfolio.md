...
- **Signature intro** (index pages): full-screen field overlay; "Chris Shi" draws
  ...
  (gitignored tooling) — public-domain Hershey Script Simplex letterforms; each
  stroke is resampled at even arc-length, Gaussian-smoothed with endpoints
  fixed, then Catmull-Rom fitted, because the font's raw integer-grid vertices
  read as choppy at full-screen scale. The flourish + viewBox are pinned in the
