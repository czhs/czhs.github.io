...
- **Signature intro** (index pages): full-screen field overlay; "Chris Shi" draws
  itself (per-stroke `stroke-dashoffset`, duration ∝ length, LINEAR easing so
  the pen moves at one constant speed — per-stroke ease-in-out read as pulsing),
  then the curtain lifts. Once per tab session (`sessionStorage rbx-sig-seen`),
  any click/key skips, reduced-motion and noscript both bypass it. The SVG is
  `_includes/portfolio_sig.liquid`, regenerated by `local/sig/sig_gen.py`
  (gitignored tooling) — public-domain Hershey Script Simplex letterforms.
  Each WORD is one path: the letters' strokes are chained into a single
  continuous pen line (glyph exits bridged to the next entry — Chris asked for
  letters attached and drawn "in a continuous line, not separately"), with the
  i-dots emitted after their word, the way a hand dots an i. Every path is
  resampled at even arc-length, Gaussian-smoothed with endpoints fixed, then
  Catmull-Rom fitted, because the font's raw integer-grid vertices read as
  choppy at full-screen scale. The flourish + viewBox are pinned in the
  generator to the hand-finished values. **Dash gotcha:** park both path
  endpoints strictly INSIDE the dash gap while hidden (oversized gap + overshot
  offset) — an endpoint exactly on a dash boundary paints a round-cap dot at
  every stroke start.
- **No ticker**: the capability ticker that shipped with the re-skin was removed
  ...
  re-add it. The sidebar still lists the skills. The sidebar and TOC rail also
...
- **Reveals**: `html.rbx-armed` lands pre-paint (only when motion allowed), an
  ...
  gates the start until the intro lifts. `[data-rv]`+`--rv` = per-element
